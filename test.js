...
test('Improvement V3 routes expose discover, community, detail, list, support and security surfaces',()=>{
 ...
 try{
  ...
  w.history.pushState(null,'','#song/track-ditto');w.biasApp.navigateTo('song',false);assert.equal(w.document.querySelector('#main-content h1').textContent,'Ditto');assert.ok(w.document.querySelector('[data-v3-item-stats]'));assert.ok(w.document.querySelector('#v3-item-lists'));
  w.history.pushState(null,'','#lists?item_key=track-ditto&item_type=song');w.biasApp.navigateTo('lists',false);assert.match(w.document.querySelector('#main-content h1').textContent,/Ditto/);assert.ok(w.document.querySelector('[data-v3-item-list-directory-body]'));
