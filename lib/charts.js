async function fetchCharts(tag,fetcher=fetch,key=process.env.LASTFM_API_KEY) {
  ...
  const response=await fetcher('https://ws.audioscrobbler.com/2.0/?'+new URLSearchParams({method:'tag.gettoptracks',tag,limit:'50',api_key:key,format:'json'}),{signal:AbortSignal.timeout(15000)});
