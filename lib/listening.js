...
async function getListening(provider, username, fetcher = fetch, apiKey = process.env.LASTFM_API_KEY) {
  ...
  for (let page=1; page<=pages; page++) {
    const params = new URLSearchParams({method:'user.gettopartists',user:username,period:'12month',limit:'500',page:String(page),api_key:apiKey,format:'json'});
  ...
  return summarize(rows, {provider,username,periodLabel:'Letzte 12 Monate',sample:false});
}
