...
function validateRelease(input) {
  ...
  if(!['draft','published'].includes(input.state)) throw error(400,'Bitte die Sichtbarkeit prüfen.');
  const events=(Array.isArray(input.events)?input.events:[]).map(event=>({date:String(event?.date || '').trim(),title:String(event?.title || '').trim()})).filter(event=>event.date && event.title).slice(0,12);
  if(events.some(event=>!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(event.date) || !Number.isFinite(Date.parse(event.date)) || event.title.length>160)) throw error(400,'Bitte bestätigte Update-Termine prüfen.');
  return {act:text('act',120,true),title:text('title',200,true),date,type:text('type',60,true),description:text('description',2000),genres:(Array.isArray(input.genres)?input.genres:[]).map(g=>String(g).trim()).filter(Boolean).slice(0,8).map(g=>g.slice(0,40)),sourceUrl:link.href,events,pipelineStep:Number(input.pipelineStep || 1),state:input.state};
}
