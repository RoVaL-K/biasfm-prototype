...
function createSpotify({clientId=process.env.SPOTIFY_CLIENT_ID,origin=process.env.APP_ORIGIN || `http://127.0.0.1:${process.env.PORT || 3000}`,fetcher=fetch}={}) {
  ...
  return {configured,origin:appOrigin,async handle(req,res,url,send) {
    ...
    if(url.pathname==='/api/spotify/connect') {
      ...
      const params=new URLSearchParams({client_id:clientId,response_type:'code',redirect_uri:redirectUri,scope:'playlist-read-private playlist-read-collaborative user-read-currently-playing user-read-recently-played',state,code_challenge_method:'S256',code_challenge:crypto.createHash('sha256').update(verifier).digest('base64url')});
    ...
    if(url.pathname==='/api/spotify/activity') {
      if(!session?.accessToken) throw fail(401,'Bitte verbinde zuerst dein Spotify-Konto.');
      const [playerResult,recentResult]=await Promise.allSettled([spotify('/me/player',session),spotify('/me/player/recently-played?limit=8',session)]);
      const map=(item,extra={})=>{const track=item?.track || item?.item || item;if(!track?.name)return null;return {title:String(track.name),artist:String(track.artists?.[0]?.name || 'Unbekannter Artist'),album:String(track.album?.name || ''),url:track.external_urls?.spotify || (track.id ? `https://open.spotify.com/track/${encodeURIComponent(track.id)}` : ''),image:String(track.album?.images?.[0]?.url || ''),nowPlaying:Boolean(extra.nowPlaying),progressMs:Number(extra.progressMs || 0),durationMs:Number(track.duration_ms || 0),playedAt:extra.playedAt || null};};
      ...
      if(playerResult.status==='fulfilled' && playerResult.value?.item && playerResult.value.is_playing) result.nowPlaying=map(playerResult.value.item,{nowPlaying:true,progressMs:playerResult.value.progress_ms}); else if(playerResult.status==='rejected') result.partial=true;
      if(recentResult.status==='fulfilled') result.recentTracks=(recentResult.value?.items || []).map(item=>map(item,{playedAt:item.played_at})).filter(Boolean).slice(0,7); else result.partial=true;
      if(!result.nowPlaying && !result.recentTracks.length && playerResult.status==='rejected' && recentResult.status==='rejected') throw playerResult.reason || recentResult.reason;
      send(200,JSON.stringify(result));return true;
    }
