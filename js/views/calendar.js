...
(function (root, factory) {
  ...
  class CalendarView {
    ...
    card(cb) {
      ...
      return `<article class="comeback-card compact-release"><div class="release-main">${biasUI.cover(cb.title,cb.act)}<div class="release-copy"><span class="pill pill-muted">${esc(cb.type)}</span><h3>${esc(cb.act)} <span class="section-note">${esc(cb.actHangul || '')}</span></h3><p>„${esc(cb.title)}“</p><small>${esc(cb.sourceName || 'Persönlicher Termin')} · ${(cb.genres || []).map(esc).join(' · ')}</small></div><button class="track-btn ${tracked?'is-active':''}" aria-pressed="${tracked}" onclick="biasCalendarView.toggleTrack('${cb.id}')">${tracked?'★ Gemerkt':'☆ Merken'}</button></div><div class="release-actions"><button class="btn btn-ghost" onclick="biasCalendarView.openDetails('${cb.id}')">Details & Links →</button><button class="btn btn-ghost" onclick="biasCalendarView.downloadSingleIcal('${cb.id}')">In meinen Kalender</button></div></article>`;
    }
    openDetails(id) {
    ...
    exportAllIcal() {
      const all = this.viewMode==='calendar'?this.calendarEntries().filter(cb=>cb.date.startsWith(this.month)):this.filteredComebacks();
