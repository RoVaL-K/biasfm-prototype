  ...
  class StatsView {
    ...
    render(container) {
      ...
      container.innerHTML = `<div class="view-stats">
        <div class="view-header stats-page-header"><div><div class="pill-row"><span class="pill pill-accent">Deine Musik in Zahlen</span></div><h1 class="view-title">Dein Korea-Mix</h1><p class="view-subtitle">Deine Musik, eingeordnet. Sieh dir eine öffentliche Hörhistorie an und entdecke ihre koreanischen Artists.</p></div>${r?.totalScrobbles ? '<button class="btn btn-accent" id="share-stats">Ergebnis teilen</button>' : ''}</div>
        <section class="stats-card import-card stats-query-card ${r ? 'has-result' : ''}" aria-labelledby="stats-query-title">
          <div class="stats-query-header"><div><span class="stats-section-kicker">${r ? 'DATENQUELLE' : 'START'}</span><h2 id="stats-query-title">${r ? 'Hörhistorie aktualisieren' : 'Mit einer Hörhistorie anfangen'}</h2><p class="section-note stats-query-intro">${r ? 'Prüfe den Zeitraum oder wechsle den Dienst, um deine Auswertung neu zu laden.' : 'Eine öffentliche Profilabfrage bestätigt nicht, dass dir das Konto gehört. Die Daten werden nicht als deine öffentliche Identität übernommen.'}</p></div><span class="stats-query-badge">${r ? 'Sitzung' : 'Nur ansehen'}</span></div>
          <form id="listening-form" class="studio-form">
            <div class="stats-query-fields"><div class="form-group"><label for="listening-provider" class="form-label">Musikprofil</label><select id="listening-provider" class="select-input"><option value="lastfm" ${this.provider === 'lastfm' ? 'selected' : ''}>Last.fm (empfohlen)</option><option value="listenbrainz" ${this.provider === 'listenbrainz' ? 'selected' : ''}>ListenBrainz</option></select></div><div class="form-group"><label for="listening-username" class="form-label">Öffentlicher Nutzername</label><input id="listening-username" class="text-input" value="${esc(this.username)}" maxlength="64" required autocomplete="username" placeholder="Dein Nutzername"></div>${this.provider==='lastfm'?`<div class="form-group"><label for="listening-period">Zeitraum</label><select id="listening-period" class="select-input">${[['7day','7 Tage'],['1month','1 Monat'],['3month','3 Monate'],['12month','1 Jahr'],['overall','Gesamter Verlauf']].map(([id,label])=>`<option value="${id}" ${this.period===id?'selected':''}>${label}</option>`).join('')}</select></div>`:''}</div>
            <p class="section-note stats-query-note">ListenBrainz: bis zu 1.000 zuletzt übermittelte Plays. Last.fm: gewählter Zeitraum, sobald der Betreiber den Zugang eingerichtet hat.</p>
