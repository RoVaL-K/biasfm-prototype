...
(function (root) {
  ...
  function reviewBodyMarkup(item) {
    ...
    const content = item.spoiler ? `<details class="v3-review-spoiler"><summary>Spoiler anzeigen</summary><p>${linked}</p></details>` : `<p>${linked}</p>`;
    return content;
  }
  ...
  function renderCommunity(container) {
    ...
    const groups = [...(state.remoteGroups || []), ...localGroups()].filter((group, index, all) => all.findIndex(item => item.id === group.id) === index).filter(group => !state.query || `${group.name} ${group.description}`.toLowerCase().includes(state.query.toLowerCase()));
    container.innerHTML = `<div class="v3-community-view"><div class="view-header v3-view-header"><div><p class="hero-eyebrow">COMMUNITY · MODERIERT</p><h1 class="view-title">Gemeinsam entdecken</h1><p class="view-subtitle">Entdeckbare, request-basierte Gruppen mit Pinboard und geteilten Musikobjekten. Livechat bleibt geschlossen, bis Moderation und Verifikation bereit sind.</p></div><button class="btn btn-accent" data-v3-create>＋ Gruppe gründen</button></div><div class="v3-community-toolbar"><div class="v3-tabbar"><button class="is-active">Gruppen entdecken</button><button type="button" data-v3-mine>Meine Gruppen</button><button type="button" data-v3-guides>Guides <span class="pill pill-muted">später</span></button></div><input class="chart-search-input" id="v3-community-search" type="search" value="${esc(state.query)}" placeholder="Artist, Genre, Stadt oder Interesse …"></div><section class="v3-community-notice"><span class="v3-notice-icon">${icon('shield')}</span><div><strong>Launch-sicherer Community-Start</strong><p>Pinboard, Join Requests und Links zuerst. Keine öffentlichen DMs, Datei-Uploads oder unmoderierten Echtzeitfeeds.</p></div></section><div class="v3-group-grid">${groups.map(group => `<article class="v3-group-card"><div class="v3-group-cover">${esc(group.name.slice(0, 2).toUpperCase())}</div><p class="hero-eyebrow">${group.seed ? 'VORSCHAU · KONZEPT' : group.joinMode === 'request' ? 'REQUEST-ONLY' : 'ÖFFENTLICH'}</p><h2>${esc(group.name)}</h2><p>${esc(group.description)}</p><div class="v3-group-meta"><span>${Number(group.memberCount || 0).toLocaleString('de-DE')} Mitglieder</span><span>${group.seed ? 'Beispielinhalt' : group.joinMode === 'request' ? 'Anfrage erforderlich' : 'Offener Beitritt'}</span></div><a class="btn btn-ghost" href="#community/${encodeURIComponent(group.id)}">${group.seed ? 'Vorschau ansehen' : 'Gruppe ansehen'} ${icon('arrow')}</a></article>`).join('') || '<div class="v3-empty-view"><h2>Keine Gruppe gefunden</h2><p>Gründe einen neuen kuratierten Raum.</p></div>'}</div><div id="v3-create-panel" hidden></div></div>`;
    ...
    container.querySelector('[data-v3-mine]').onclick = () => root.biasApp.showToast('Deine Gruppen erscheinen nach dem ersten Beitritt hier.');
  ...
  function renderGroup(container, groupId) {
    ...
    if (!preview && !loadedGroups[loadedKey]) {
      ...
      const inviteQuery = inviteTokenValue ? `&invite=${encodeURIComponent(inviteTokenValue)}` : '';
      api(`api/community?group=${encodeURIComponent(groupId)}${inviteQuery}`).then(result => { if (result.posts) { state.posts = {...state.posts, [groupId]: result.posts}; state.memberships[groupId] = result.membership; state.canViewPosts[groupId] = result.canViewPosts !== false; state.inviteValid[groupId] = result.inviteValid === true; state.joinRequests[groupId] = result.joinRequests || []; state.moderationLog[groupId] = result.moderationLog || []; renderGroup(container, groupId); } }).catch(() => {});
    }
  }
  ...
  function renderSharedList(container, shareSlug) {
    ...
    const paint = list => { if (!list) return renderMissing(container, 'Liste'); const items = list.items || []; container.innerHTML = `<div class="v3-lists-view"><a class="v3-back-link" href="#lists">${icon('back')} Meine Listen</a><header class="v3-release-header"><div class="v3-group-cover large">☷</div><div><p class="hero-eyebrow">GETEILTE LISTE · ${esc(list.visibility || 'public')}</p><h1>${esc(list.title)}</h1><p>${esc(list.description || 'Eine kuratierte Sammlung aus der bias.fm-Community.')}</p><span class="section-note">${items.length} Einträge · geteilt über bias.fm</span></div></header><section class="settings-card v3-shared-list-items"><div class="section-heading-row"><div><p class="hero-eyebrow">TRACKLISTE</p><h2>Gespeicherte Einträge</h2></div><span class="section-note">Quelle bleibt beim Originalanbieter</span></div>${items.map(item => `<div class="v3-saved-track"><span><strong>${esc(item.title || item.entityId || 'Eintrag')}</strong><small>${esc(item.artistName || '')}</small></span>${item.entityId ? `<a class="btn btn-ghost btn-sm" href="#song/${encodeURIComponent(item.entityId)}">Öffnen ${icon('arrow')}</a>` : ''}</div>`).join('') || '<p class="section-note">Diese Liste enthält noch keine Einträge.</p>'}</section></div>`; };
  ...
  root.biasV3 = {discover:{tab:'artists',query:'',scene:'Alle',genre:'Alle',type:'Alle',period:'Alle',sort:'Relevanz',pageSize:100}, community:{groupId:'',query:'',loaded:false,posts:{},loadedGroups:{},memberships:{},canViewPosts:{},joinRequests:{},moderationLog:{},inviteTokens:{},inviteValid:{},lookupPending:{}}, lists:{}};
