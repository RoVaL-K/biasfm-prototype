(function(root){
  ...
  function cover(title,artist,large=false){
    ...
    return `<span class="artwork ${large?'artwork-large':''}" role="img" title="Eigenes Ersatzmotiv, kein Originalcover" aria-label="Cover: ${esc(title)} von ${esc(artist)}" style="--art-hue:${hue}"><span class="artwork-disc"></span><span class="artwork-type">${esc(String(title).slice(0,2).toUpperCase())}</span></span>`;
  }
