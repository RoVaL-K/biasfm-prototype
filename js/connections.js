(function(root){
  ...
  const connections={
    ...
    renderListening(){
      ...
      const preference=['lastfm','spotify'].includes(biasStore.profile?.listeningProvider) ? biasStore.profile.listeningProvider : 'lastfm';
      const preferred=preference==='spotify'?spotify:lastfm;
      const source=preferred && (preferred.nowPlaying || preferred.recentTracks?.length) ? preferred : ((lastfm && (lastfm.nowPlaying || lastfm.recentTracks?.length)) ? lastfm : (spotify || lastfm));
