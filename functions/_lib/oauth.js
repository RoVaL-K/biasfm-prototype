...
async function accountForIdentity(env, provider, identity) {
  ...
  try {
    await env.DB.prepare(`INSERT INTO accounts (id, email, username, password_hash, profile_json, privacy_json, notification_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .bind(id, identity.email, username, `oauth$${provider}$${await randomId(24)}`, JSON.stringify(profile), JSON.stringify({profile: 'public', stats: 'private', activity: 'private', follows: 'public', favorites: 'public', showNowPlaying: false}), JSON.stringify({release: true, announcement: false, reminder: true, social: false, product: true}), now, now).run();
    await env.DB.prepare(`INSERT INTO account_activity_privacy (account_id, visibility, show_now_playing, updated_at) VALUES (?, 'private', 0, ?)`)
      .bind(id, now).run();
  } catch (error) {
